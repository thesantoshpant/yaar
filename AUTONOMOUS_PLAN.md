...
## Mock-test expansion backlog (autonomous, ~1h)
- [x] Reading (adaptive generate + objective score + history + memory). Guest-score 500 fixed.
- [ ] Writing: generate real task (IELTS Task 2 essay; TOEFL Academic Discussion), Gemini-score vs rubric -> band/scaled, save history + memory weak-criteria.
- [ ] Speaking: generate task(s) (IELTS parts / TOEFL tasks), score from transcript (recorder), band/scaled, history + memory.
- [ ] Listening: generate transcript + MCQs (cache key), browser reads aloud (speechSynthesis), objective score like reading, history + memory.
- [ ] Frontend: Mock page section picker (Reading/Writing/Speaking/Listening) + a runner for each, design-consistent; history shows all skills.
- [ ] Keep smoke green; add writing/speaking/listening to smoke. Commit per section.
